...
class Hebbian(object):
    ...
    def train(self, X, y, batch_size=1, num_epochs=10, alpha=0.1, gamma=0.9, learning="Delta"):
        ...
        """
        X = np.insert(X, 0, 1, axis = 0)
        iteration =  X.shape[1] // batch_size
        for epoch in range(num_epochs):
            for i in range(iteration+1):
                Xtrain_batch = X[:,i*batch_size:batch_size+(i*batch_size)]
                ytrain_batch = y[i*batch_size:batch_size+(i*batch_size)]
                net = np.dot(self.weights,Xtrain_batch)
                if self.transfer_function == "Hard_limit":
                    y_out = np.where(net >= 0.0, 1.0, 0.0)
                elif self.transfer_function == "Linear":
                    y_out = net
                elif self.transfer_function == "Sigmoid":
                    y_out = 1/(1+np.exp(-net))
                size = ytrain_batch.shape[0]
                ytrain_batch = ytrain_batch.reshape(1, size)
                Y_new = np.eye(10)[ytrain_batch.astype('int32')]
                Y_new = Y_new.T.reshape(10, size)
                error = Y_new - y_out
                if learning == "Delta":
                    self.weights = self.weights + alpha*np.dot(error,Xtrain_batch.T)
                elif learning == "Filtered":
                    self.weights = (1-gamma)*self.weights + alpha*np.dot(Y_new,Xtrain_batch.T)
                elif learning == "Unsupervised_hebb":
                    self.weights = self.weights + alpha*np.dot(y_out,Xtrain_batch.T)
    ...
    def calculate_percent_error(self, X, y):
        ...
        num_of_errors, correct = 0, 0 
        for i in range(X.shape[1]):
            i_y_pred = np.argmax(y_hat[:,i])
            i_y_true = y[i]
            if i_y_pred == i_y_true:
                correct += 1
                continue
            else:
                num_of_errors += 1
    ...
    def calculate_confusion_matrix(self, X, y):
        ...
        """
        y_hat = self.predict(X)
        c_matrix = np.zeros((self.number_of_classes,self.number_of_classes))
        for i in range(X.shape[1]):
            i_y_pred = np.argmax(y_hat[:,i])
            i_y_true = y[i]
            c_matrix[i_y_true, i_y_pred] += 1          
        return c_matrix
